"""Validate the archived DS2 penalty runs and reconstruct test metrics."""
...
def main() -> None:
    ...
    frame.to_csv(output_dir / "legacy_ds2_penalty_metrics.csv", index=False)
    ...
    pd.DataFrame(summary_rows).to_csv(
        output_dir / "legacy_ds2_penalty_summary.csv", index=False
    )
    (output_dir / "legacy_ds2_penalty_validation.json").write_text(
        json.dumps(
            {
                "status": "PASS",
    ...
    print("PASS: 15 archived DS2 runs validated and metrics reconstructed")
